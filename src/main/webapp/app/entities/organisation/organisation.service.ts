...
export class OrganisationService {
  ...
  protected convertDateFromClient(organisation: IOrganisation): IOrganisation {
    const copy: IOrganisation = Object.assign({}, organisation, {
      dateCreation: organisation.dateCreation && organisation.dateCreation.isValid() ? organisation.dateCreation.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((organisation: IOrganisation) => {
        organisation.dateCreation = organisation.dateCreation ? moment(organisation.dateCreation) : undefined;
      });
    }
    return res;
  }
